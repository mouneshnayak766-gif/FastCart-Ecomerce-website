...
export default function Account() {
  ...
  const logout = async () => {
    try {
      ...
      console.log("Logout backend error (ignored):", e);
    } finally {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      triggerMessage("error", err.response?.data || "Failed to update profile.");
    }
  };
  ...
  const handleChangePasswordSubmit = async (e) => {
    ...
    try {
      ...
      triggerMessage("error", err.response?.data || "Failed to change password.");
    }
  };
    ...
    <div className="bg-gray-100 min-h-screen p-8">
      <div className="max-w-5xl mx-auto bg-white p-8 rounded-2xl shadow-lg">
        ...
        <div className="flex gap-3 mb-6">
          <button
            onClick={() => navigate("/")}
            className="px-4 py-1.5 rounded-full bg-blue-600 text-white text-sm hover:bg-blue-700"
          >
            Home
          </button>
        </div>
